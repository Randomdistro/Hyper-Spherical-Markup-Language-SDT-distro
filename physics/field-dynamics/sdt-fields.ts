...
import { SphericalCoordinate, State21D, SDTCore, SDTConstants, SDT21DState, state21DToSpherical } from '../sdt-engine/sdt-core';
...
export class SDTFieldDynamics {
   ...
   */
  static calculateAuthentic21DField(
    system: Authentic21DMultiBodySystem,
    fieldPoint: SphericalCoordinate
  ): SDTField {
   ...
   */
  private static calculate21DFluxContribution(state21D: SDT21DState, distance: number): number {
   ...
   */
  private static calculate21DDirectionalModifier(state21D: SDT21DState): [number, number, number] {
  ...
  private static calculate21DInteraction(
    body1: { mass: number; position: SphericalCoordinate; state21D: SDT21DState },
    body2: { mass: number; position: SphericalCoordinate; state21D: SDT21DState },
    fieldPoint: SphericalCoordinate
  ): number {
   ...
   */
  private static calculate21DResonance(state1: SDT21DState, state2: SDT21DState): number {
   ...
   */
  private static calculate21DPhaseCoherence(state1: SDT21DState, state2: SDT21DState): number {
  ...
  static calculateTotalField(
    ...
    for (let i = 0; i < system.bodies.length; i++) {
      for (let j = i + 1; j < system.bodies.length; j++) {
        ...
        const eclipsing = SDTCore.calculateEclipsing(
          r12, body1.mass, body2.mass, omega12, omega21
        );
   ...
   */
  static calculateElectromagneticField(
    state21D: State21D,
    charge: number
  ): {
    ...
    const electric: [number, number, number] = [
    ...
    const magnetic: [number, number, number] = [
   ...
   */
  static calculateQuantumTransition(
    distance: number,
    state21D: State21D
  ): number {
    // χ(r) = exp(-r²/2λ_q²)
