...
import { SDTSphericalMath, SDTUserPhysics } from './sdt-spherical-math';
export class SDTHSMLElement {
    elementState;
    children = [];
    parent = null;
    domElement;
    constructor(tagName, initialPosition, matterState = MatterState.SOLID, mass = 1.0) {
    ...
    initializePosition21D(state21D, position) {
    ...
    updatePhysics(dt, globalSystem) {
        ...
        this.elementState.state21D = evolve21DState(this.elementState.state21D, field.displacement, field.pressure, dt);
        ...
        this.elementState.acceleration = SDTFieldDynamics.calculateGravitationalAcceleration(field, this.elementState.mass);
     ...
     */
    calculateElementSteradianSize(coord) {
    ...
    updateMaterialProperties() {
        const properties = SDTMatterStates.calculateMaterialProperties(this.elementState.state21D, this.elementState.matterState);
...
export class SDTHSMLDocument {
     ...
     */
    createElement(tagName, position, matterState = MatterState.SOLID, mass = 1.0) {
        ...
        const animate = (currentTime) => {
            ...
            const system = {
                bodies: Array.from(this.elements.values()).map(el => {
                    ...
                    const legacyState21D = {
                        level0: [state.state21D.ξ0, 0, 0],
                        level1: [state.state21D.ξL[0], state.state21D.ξL[1], 0],
                        level2: state.state21D.ξP,
                        level3: [state.state21D.ξS[0], state.state21D.ξS[1], state.state21D.ξS[2]],
                        level4: [...state.state21D.φ, 0],
                        level5: [state.state21D.ε[0], state.state21D.ε[1], state.state21D.ε[2]]
                    };
                    return {
                        mass: state.mass,
    ...
    checkUserCollisions() {
        ...
        this.elements.forEach(element => {
            ...
            const safeElementPos = SDTSphericalMath.toSafeSpherical(elementState.position.r, elementState.position.theta, elementState.position.phi);
            const collision = SDTUserPhysics.calculateUserCollision(this.userState, safeElementPos, elementState.mass * 0.1 // Approximate element radius from mass
            );
     ...
     */
    getViewportSteradianSlice(fovDegrees = 60) {
     ...
     */
    screenToSphericalRay(screenX, screenY) {
        const viewport = this.getViewportSteradianSlice();
        return SDTSphericalMath.pixelToSphericalDirection(screenX, screenY, viewport.angularBounds);
    }
}
//# sourceMappingURL=sdt-hsml-dom.js.map