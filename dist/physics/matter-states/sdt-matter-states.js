 ...
 */
import { SDTCore } from '../sdt-engine/sdt-core';
...
export class SDTMatterStates {
     ...
     */
    static getStateProperties(state) {
        switch (state) {
     ...
     */
    static calculatePhaseTransition(currentState, state21D, temperature) {
        ...
        if (currentState === MatterState.SOLID) {
            ...
            if (oscillatoryEnergy > 200 && thermalMotion > 0.5)
     ...
     */
    static calculateLegacyPhaseTransition(currentState, state21D, temperature) {
     ...
     */
    static applyStateEffects(state21D, matterState, dt) {
        const props = this.getStateProperties(matterState);
        // Deep copy to avoid mutation
        const newState = JSON.parse(JSON.stringify(state21D));
     ...
     */
    static applyLegacyStateEffects(state21D, matterState, dt) {
        const props = this.getStateProperties(matterState);
        // Modify state based on matter properties
        const newState = { ...state21D };
        // Apply vibrational effects to Level 4 (Oscillation)
        newState.level4 = newState.level4.map((v, i) => v + props.vibrationalAmplitude * Math.sin(Date.now() * 0.001 + i) * dt);
        // Apply cohesion effects to Level 2 (Plane - relationships)
        newState.level2 = newState.level2.map(v => v * (1 + props.cohesionStrength * dt * 0.01));
        // Apply resistance effects to Level 1 (Line - movement)
        newState.level1 = newState.level1.map(v => v * (1 - props.spationFluxResistance * dt * 0.001));
        return newState;
    }
    /**
     * Calculate material properties from authentic 21D SDT state
     */
    static calculateMaterialProperties(state21D, matterState) {
        const props = this.getStateProperties(matterState);
     ...
     */
    static calculateLegacyMaterialProperties(state21D, matterState) {
        const props = this.getStateProperties(matterState);
...
//# sourceMappingURL=sdt-matter-states.js.map